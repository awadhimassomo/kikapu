...
class NameAliasMixin:
    ...
    def get_real_name(self):
        ...
        return self._get_firstName(dot_idx, real_name=True)
    ...
    def get_alias(self):
        ...
        if kw is not None:
            return self._get_firstName(kw_idx + 1, keywords=True)

        # "name alias" or "complicated column expression alias"
        _, ws = self.token_next_by(t=T.Whitespace)
        if len(self.tokens) > 2 and ws is not None:
            return self._get_firstName(reverse=True)
...
class TokenList(Token):
    ...
    def _get_firstName(self, idx=None, reverse=False, keywords=False,
                        real_name=False):
